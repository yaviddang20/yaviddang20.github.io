    ...
    _validateMapChunk(code, propName, chunkName, chunks) {
        Debug.call(()=>{
            ...
            [
                [
                    '$UV',
                    `{STD_${propName}_TEXTURE_UV}`
                ],
                [
                    '$CH',
                    `{STD_${propName}_TEXTURE_CHANNEL}`
                ],
                [
                    '$SAMPLER',
                    `{STD_${propName}_TEXTURE_NAME}`
                ],
                [
                    '$DECODE',
                    `{STD_${propName}_TEXTURE_DECODE}`
                ],
                [
                    '$VC',
                    `{STD_${propName}_VERTEX_CHANNEL}`
                ],
                [
                    '$DETAILMODE',
                    `{STD_${propName}_DETAILMODE}`
                ],
                [
                    'unpackNormal(',
                    `{STD_${propName}_TEXTURE_DECODE}(`
            ...
            [
                ...
                [
                    'MAPVERTEX',
                    `STD_${propName}_VERTEX`
                ],
                [
                    'MAPTEXTURE',
                    `STD_${propName}_TEXTURE`
                ],
                [
                    'MAPINVERT',
                    `STD_${propName}_INVERT`
                ]
            ].forEach(([oldSyntax, newSyntax])=>trackChange(oldSyntax, newSyntax));
            // custom handling
            ...
            if (chunkCode) {
                ...
                if (vertexColorOption) {
                    if (code.includes('gammaCorrectInputVec3(saturate3(vVertexColor.') || code.includes('gammaCorrectInput(saturate(vVertexColor.')) {
                        Debug.errorOnce(`Shader chunk ${chunkName} contains gamma correction code which is incompatible with vertexColorGamma=true. Please remove gamma correction calls from the chunk.`, {
                            code: code
                        });
                    }
                }
